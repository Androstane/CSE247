...
if __name__ == "__main__":
	...
	ap.add_argument("-iter","--number of iterations",required=True, help="Number of iterations in search for optimal tree")
	...
	ap.add_argument("-gt","--path to ground truth",required=False, help="Path to the newick string containing the ground truth tree")
	...
	seqs_parsed, names = parse.Parse_WG(filename=in_path)
	...
	print my_dict
	string_frst, hier_tr = tree.compute_tree(seqs_)
	...
	for iteration in range(T):
		...
		if len(stack_)>max_stack_len:
			stack_ = dict(stack_.items()[:max_stack_len]) 
	...
	# 	tree_format.unroot()
	# 	print(tree_format)
	# 	print(Check_Tree(tr_key).get_topology_id())
	# 	print(tree_format.get_topology_id())
	# 	print(stack_[tr_key])
	# 	print(groundtruth_tr.robinson_foulds(tree_format, unrooted_trees=True)[0])
	plt.plot(scores, 'o-')
